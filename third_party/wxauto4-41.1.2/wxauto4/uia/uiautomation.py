...
class Control():
    ...
    def FindAll(
        ...
        for i in range(result.Length):
            try:
                sub_control = Control.CreateControlFromElement(result.GetElement(i))
    ...
    def MiddleClick(self, x: int = None, y: int = None, ratioX: float = 0.5, ratioY: float = 0.5, simulateMove: bool = True, waitTime: float = OPERATION_WAIT_TIME, move: bool=False, pos='center', show_window=False) -> None:
        """
        ...
        if move:
            ...
            if not hasattr(self, 'winapi'):
                self.winapi = Win32(self.GetTopLevelControl().NativeWindowHandle)
            self.winapi.click_by_bbox(self.BoundingRectangle, button='mid', xbias=x, ybias=y, pos=pos, activate=show_window)
    ...
    def WheelDown(self, x: int = None, y: int = None, ratioX: float = 0.5, ratioY: float = 0.5, wheelTimes: int = 1, interval: float = 0.05, waitTime: float = OPERATION_WAIT_TIME, api=True, use_safe_coord=True) -> None:
        """
        ...
        interval: float.
        waitTime: float.
        use_safe_coord: bool, if True (default), use coordinates away from window edges to avoid triggering window resize.
        """
        if api:
            if not hasattr(self, 'winapi'):
                self.winapi = Win32(self.GetTopLevelControl().NativeWindowHandle)
            self.winapi.scroll_wheel(self.BoundingRectangle, wheelTimes*(-120), use_safe_coord=use_safe_coord)
        else:
    ...
    def WheelUp(self, x: int = None, y: int = None, ratioX: float = 0.5, ratioY: float = 0.5, wheelTimes: int = 1, interval: float = 0.05, waitTime: float = OPERATION_WAIT_TIME, api=True, use_safe_coord=True) -> None:
        """
    ...
    def Paste(self, text: str, click=True):
        pyperclip.copy(text)
        self.ShortcutPaste(click=click)
